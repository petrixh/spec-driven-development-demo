...
describe('MovieDetailView', () => {
  it('loads the selected movie and shows showtimes grouped by date', async () => {
  ...
  it('shows empty state when no showtimes available', async () => {
    const { fetchMovieDetails } = await import('Frontend/lib/movies-client');
    vi.mocked(fetchMovieDetails).mockResolvedValue({
      id: 1,
      title: 'No Shows Movie',
    ...
    await waitFor(() =>
      expect(
        screen.getByText('No future showtimes are currently scheduled for this movie.'),
      ).toBeInTheDocument(),
    );
  });
});
